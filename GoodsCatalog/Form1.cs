    ...
    public partial class Form1 : Form
    {
        ...
        private void AuthUser()
        {
            ...
            if (loginWindow.ShowDialog() == DialogResult.OK)
            {
                ...
                string sqlQuery = $"select Login,Password from Users im nbv 1 where Login='{login}' and Password='{passw}'";
                try
                {
                    connection.Open();
        ...
        private void createCategoryItem_Click(object sender, EventArgs e)
        {
            ...
            {
                //Для избежания Sql-иньекций запрещено так подставлять значения переменных
                //прямо в строку запроса => для этого надо использовать специальные параметры
                //(параметризованные запросы) !!!
                //string query = $"insert into Categories (Name) values ({categoriesEditor.CategoryName})";
